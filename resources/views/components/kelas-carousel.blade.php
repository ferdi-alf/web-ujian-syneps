    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col sm:flex-row justify-between items-center mb-6 sm:mb-8">
            <div class="text-center sm:text-left mb-4 sm:mb-0">
                <h2 class="text-xs sm:text-sm font-semibold tracking-wide text-emerald-500 uppercase">Program Unggulan
                </h2>
                <h3 class="mt-2 text-xl sm:text-2xl lg:text-3xl font-extrabold text-gray-900">Pilih Kelas Terbaik Untukmu
                </h3>
            </div>
        </div>
                        ...
                        <div
                            ...
                            <div class="relative z-10 h-full flex flex-col">
                                <div class="p-4 sm:p-5">
                                    <div class="flex items-start justify-between mb-3">
                                        <div class="flex-1">
                                            <div class="flex items-center gap-2 mb-2">
                                                <div
                                                    class="w-7 h-7 sm:w-8 sm:h-8 bg-gradient-to-br from-emerald-400 to-blue-500 rounded-lg flex items-center justify-center">
                                    ...
                                    <div
                                        class="bg-gradient-to-r from-gray-50 to-emerald-50 rounded-xl p-3 sm:p-4 mb-3 sm:mb-4">
                                        <div class="flex items-center justify-between mb-2">
                                            @if (is_numeric($kelasItem['harga']))
                                                <span class="text-xs sm:text-sm text-gray-500 line-through">Rp
                                                    {{ number_format($kelasItem['harga'] + 500000, 0, ',', '.') }}</span>
                                            @endif
                                            @if ($kelasItem['dp'] > 0)
                                                <span
                                                    class="bg-gradient-to-r from-emerald-500 to-teal-500 text-white text-xs font-bold px-2 sm:px-3 py-1 rounded-full shadow-md">DP
                                                    {{ $kelasItem['dp'] }}%</span>
                                            @endif
                                        </div>
                                        <div class="flex items-baseline gap-1">
                                            <span
                                                class="text-xl sm:text-2xl font-bold bg-gradient-to-r from-emerald-600 to-teal-600 bg-clip-text text-transparent">{{ is_numeric($kelasItem['harga']) ? 'Rp ' . number_format($kelasItem['harga'], 0, ',', '.') : $kelasItem['harga'] }}</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="px-4 sm:px-5 flex-grow">
                                    <div class="space-y-2 sm:space-y-3">
                                        <div class="flex items-center gap-2 sm:gap-3 text-xs sm:text-sm text-gray-700">
                                            <div
                                                class="w-5 h-5 sm:w-6 sm:h-6 bg-emerald-100 rounded-full flex items-center justify-center flex-shrink-0">
                                        ...
                                        <div class="flex items-center gap-2 sm:gap-3 text-xs sm:text-sm text-gray-700">
                                            <div
                                                class="w-5 h-5 sm:w-6 sm:h-6 bg-purple-100 rounded-full flex items-center justify-center flex-shrink-0">
                                        ...
                                        <div class="flex items-center gap-2 sm:gap-3 text-xs sm:text-sm text-gray-700">
                                            <div
                                                class="w-5 h-5 sm:w-6 sm:h-6 bg-orange-100 rounded-full flex items-center justify-center flex-shrink-0">
                                ...
                                <div class="p-4 sm:p-5 mt-auto">
                                    ...
                                    <a href="{{ route('kelas.detail', $slug) }}"
                                        ...
                                        <span class="flex items-center justify-center gap-2">
                                            Lihat Detail
                                            <i
                                                class="fas fa-arrow-right text-xs sm:text-sm group-hover:translate-x-1 transition-transform"></i>
                                        </span>
                                    </a>
                                    <div class="admin-actions hidden mt-2 sm:mt-3" data-admin-actions>
                                        <div class="flex gap-2">
                                            <button
                                                ...
                                                class="flex-1 bg-blue-500 hover:bg-blue-600 text-white px-2 sm:px-3 py-1 sm:py-2 rounded-lg text-xs font-medium transition-colors">
                                                <i class="fas fa-edit mr-1"></i>Edit
                                            </button>
                                            <button
                                                onclick="deleteKelas('{{ $kelasItem['id'] ?? 'temp-' . $loop->index }}')"
                                                class="flex-1 bg-red-500 hover:bg-red-600 text-white px-2 sm:px-3 py-1 sm:py-2 rounded-lg text-xs font-medium transition-colors">
    ...
    <script>
        ...
        function initializeSwiper() {
            ...
            swiperInstance = new Swiper('.mySwiper', {
                ...
                autoplay: false, // Autoplay disabled since no loop
                slidesPerView: 3,
                spaceBetween: 24,
                initialSlide: totalSlides > 1 ? 1 : 0, // Start from middle slide if available
                speed: 700,
                watchOverflow: true,
                breakpoints: {
                    320: {
                ...
                on: {
                    init: function() {
                        console.log('Swiper initialized successfully with', totalSlides, 'slides');
                    }
                }
            });
        }
